...
%
% Discretizzazione
dt     = 0.005;                        % Time step, deve essere un divisore di tend  
...
for n = 1:NMAX
    ...
    % Metodo di RK4
    k1 = f(t(n),Z(n,:));
    k2 = f(t(n)+0.5*dt, Z(n,:)+0.5*dt*k1);
    k3 = f(t(n)+0.5*dt, Z(n,:)+0.5*dt*k2);
    k4 = f(t(n)+1.0*dt, Z(n,:)+1.0*dt*k3);
...
figure('Name','Simulazione con metodo di RK4');
subplot(2,1,1)
...
%
% Confronto tra la soluzione esatta di RK4 e la soluzione di RK4
% approssimata
...
xlabel('Tempo (s)');
ylabel('Angolo (rad)');
legend('\theta_1 RK4', '\theta_2 RK4', '\theta_1 RK4e', '\theta_2 RK4e')

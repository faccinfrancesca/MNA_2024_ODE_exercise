...
for n=1:NMAX
    ...
    %
    % Metodo SDIRK
    k_guess = NewtonS(Z(n,:),t(n),dt);     % Calcolare k1 e k2 con il metodo di Newton
    %
    k1 = k_guess(1:4);
    k2 = k_guess(5:8);
    %
    Z(n+1, :) = Z(n, :) + dt/2*(k1+k2);     % Calcolo di Z(n+1)
    %
    ...
    %
    t(n+1) = t(n) + dt;         % Aggiornamento del tempo
end
%
%% CALCOLO DELL'ERRORE
%
...
figure('Name','Simulazione con metodo SDIRK');
subplot(2,1,1)
...
%
% Confronto tra la soluzione esatta di RK4 e la soluzione di SDIRK
figure('Name','Confronto tra gli angoli');
plot(t,Z(:,1),'b--', t,Z(:,3),'r--', te,Ze(:,1),'b-', te,Ze(:,3),'r-');
hold on 
xlabel('Tempo (s)');
ylabel('Angolo (rad)');
legend('\theta_1 SDIRK', '\theta_2 SDIRK', '\theta_1 RK4e', '\theta_2 RK4e')

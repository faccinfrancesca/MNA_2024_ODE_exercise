...
indice = 1;    
% Attenzione! Per poter eseguire un altro test è necessario modificare 
% anche l'indice nella funzione "Newton.m".
%
...
%
% Discretizzazione
dt      = 0.1;                         % Time step, deve essere un divisore di tend  
...
figure('Name','Simulazione con metodo di Crank-Nicolson');
subplot(2,1,1)
...
%
% Confronto tra la soluzione esatta di RK4 e la soluzione di Crank_Nicolson
figure('Name','Confronto tra gli angoli');
plot(t,Z(:,1),'b--', t,Z(:,3),'r--', te,Ze(:,1),'b-', te,Ze(:,3),'r-');
hold on 
xlabel('Tempo (s)');
ylabel('Angolo (rad)');
legend('\theta_1 CN', '\theta_2 CN', '\theta_1 RK4e', '\theta_2 RK4e')
title(['Confronto angoli - test ', num2str(indice)])
%
figure('Name','Velocità');
plot(t,Z(:,2),'b--', t,Z(:,4),'r--', te,Ze(:,2),'b-', te,Ze(:,4),'r-');
hold on 
xlabel('Tempo (s)');
ylabel('Angolo (rad)');
legend('\theta_1 CN', '\theta_2 CN', '\theta_1 RK4e', '\theta_2 RK4e')
